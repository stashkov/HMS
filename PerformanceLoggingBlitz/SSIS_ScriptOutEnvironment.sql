...
DECLARE
    @folder_name nvarchar(200)              = 'epi_hms',
    @environment_name_current nvarchar(200) = 'DEV',
    @environment_name_new nvarchar(200)     = 'Dev',
    @name sysname, 
    @sensitive bit, 
...
--IF NOT EXISTS (SELECT 1 FROM [SSISDB].[catalog].[folders] WHERE name = N'epi_hms')
--    EXEC [SSISDB].[catalog].[create_folder] @folder_name=N'epi_hms', @folder_id=@folder_id OUTPUT
--ELSE
--    SET @folder_id = (SELECT folder_id FROM [SSISDB].[catalog].[folders] WHERE name = N'epi_hms')
 
--IF NOT EXISTS (SELECT 1 FROM [SSISDB].[catalog].[environments] WHERE folder_id = @folder_id AND name = N'Dev')
--    EXEC [SSISDB].[catalog].[create_environment] @environment_name=N'Dev', @folder_name=N'epi_hms'
 
--SET @environment_id = (SELECT environment_id FROM [SSISDB].[catalog].[environments] WHERE folder_id = @folder_id and name = N'Dev')
 
--DECLARE @var sql_variant
 
--SET @var = N'Data Source=.\EPITOME;Initial Catalog=hotel;Provider=SQLNCLI11.1;Integrated Security=SSPI;Auto Translate=False;'
--IF NOT EXISTS (SELECT 1 FROM [SSISDB].[catalog].[environment_variables] WHERE environment_id = @environment_id AND name = N'ConnStrEPI')
--            EXEC [SSISDB].[catalog].[create_environment_variable] @variable_name=N'ConnStrEPI', @sensitive=0, @description=N'', @environment_name=N'Dev', @folder_name=N'epi_hms', @value=@var, @data_type=N'String'
 
--SET @var = N'Data Source=.;Initial Catalog=VEGAUAT;Provider=SQLNCLI11.1;Integrated Security=SSPI;Auto Translate=False;'
--IF NOT EXISTS (SELECT 1 FROM [SSISDB].[catalog].[environment_variables] WHERE environment_id = @environment_id AND name = N'ConnStrHMS')
--            EXEC [SSISDB].[catalog].[create_environment_variable] @variable_name=N'ConnStrHMS', @sensitive=0, @description=N'', @environment_name=N'Dev', @folder_name=N'epi_hms', @value=@var, @data_type=N'String'