BEGIN TRANSACTION;
--current status of the process  -- can be ('reserve', 'assignroom', 'checkin', 'checkout', 'cancel')
DECLARE @status NVARCHAR(10);
SET @status = N'checkout';
...
SET @AdultCount = 1;
 -- should be in accordance with the number of guests, but 1 is always acceptable
...
SET @ConfirmationNumber = @TrackingNumber
...
-------------------check out
--everything necessary already declared
...
IF @status IN ( 'reserve', 'cancel', 'assignroom', 'checkin', 'checkout' )
    BEGIN
...
IF @status IN ( 'cancel' )
    BEGIN
        SELECT  'TODO: implement cancel';
    END;

IF @status IN ( 'assignroom', 'checkin', 'checkout' )
    BEGIN
