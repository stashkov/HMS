...
/****** Object:  StoredProcedure [dbo].[sp_epi_group_assign_room_types]    Script Date: 05-Oct-16 1:35:54 PM ******/
SET ANSI_NULLS ON
GO
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_epi_group_assign_room_types]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[sp_epi_group_assign_room_types] AS' 
END
GO

ALTER PROCEDURE [dbo].[sp_epi_group_assign_room_types]

 @DayDate DATETIME, --= '20160715'
 @RoomType NVARCHAR(10),-- = N'SRTS'
 @PropGroupBookingID INT --= 5
...
/****** Object:  StoredProcedure [dbo].[sp_epi_create_group]    Script Date: 05-Oct-16 1:36:21 PM ******/
SET ANSI_NULLS ON
GO
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_epi_create_group]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[sp_epi_create_group] AS' 
END
GO

ALTER PROCEDURE [dbo].[sp_epi_create_group]

    @GroupName NVARCHAR(100), --= N'mhuhuua000555'
    @RatePlanCode NVARCHAR(6), --= N'BOOK'
    @CheckInDate DATETIME, --= '20160709'
    @CheckOutDate DATETIME, --= '20160715'
    @ReleaseDate DATETIME, --= '20160714'
    @SourceCode NVARCHAR(6), --= N'CALL'
    @PropGroupBookingID INT = NULL OUT,
...
AS
    ...
    SET @PropGroupBookingID = SCOPE_IDENTITY()
    ...
    INSERT INTO [dbo].[PackageInfo]
       ...
       IF @PropGroupBookingID IS NULL
       BEGIN
          DECLARE @msg NVARCHAR(2000) = 'PostExecution check failed. At least one of the parameters is null;'
...
/****** Object:  StoredProcedure [dbo].[sp_epi_group_assign_rates]    Script Date: 05-Oct-16 1:36:11 PM ******/
SET ANSI_NULLS ON
GO
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_epi_group_assign_rates]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[sp_epi_group_assign_rates] AS' 
END
GO


ALTER PROCEDURE [dbo].[sp_epi_group_assign_rates]

 @CheckInDate DATETIME, --= '20160715'
 @CheckOutDate DATETIME, --= '20160715'
 @RoomType NVARCHAR(10),-- = N'SRTS'
 @RatePlanCode NVARCHAR(6), --= N'BOOK'
 @PropGroupBookingID INT
...
DECLARE @PropertyRatePlanID INT = (select PropertyRatePlanID
                                  from dbo.PropertyRatePlan
                                  WHERE propGroupBookingID = @PropGroupBookingID)
...
EXEC prc_GetFlatRatesRange @PropertyCode,@CheckInDate,@CheckOutDate,N'RUB',N'RUB',@LengthOfStayCode,@RatePlanCode,N'',@RoomType,1 
--EXEC prc_GetFlatRatesRange N'VEGA','20160630','20160715',N'RUB',N'RUB',N'DY',N'BOOK',N'','SRTS',1 
